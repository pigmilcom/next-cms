...
const initializeDatabase = async () => {
    ...
    try {
        ...
        // Check and create users with default admin
        try {
            ...
            if (!hasData) {
                // Create default admin user - following auth handler pattern
                const { userData: defaultUser, credentials } = await getDefaultUser();
                const createResult = await DBService.create(defaultUser, 'users');
                if (createResult?.success) {
                    results.tablesCreated.push('users');
                    results.defaultCredentials = credentials;
                    
                    // Send welcome email - same as auth handler
                    try {
                        await sendWelcomeEmail(defaultUser.email, defaultUser.displayName);
                    } catch (e) {
                        console.log('Welcome email failed:', e.message);
                    }
                } else {
                    results.errors.push('Failed to create default admin user: ' + (createResult?.error || 'Unknown error'));
                }
            }
        } catch (error) {
            console.log('Creating users table with default admin...', error.message);
            try {
                // Create default admin user - following auth handler pattern
                const { userData: defaultUser, credentials } = await getDefaultUser();
                const createResult = await DBService.create(defaultUser, 'users');
                ...
                results.errors.push('Failed to create default admin user: ' + createError.message);
            }
        }
