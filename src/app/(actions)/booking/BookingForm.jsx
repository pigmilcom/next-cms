...
import { FaCalendar, FaChevronLeft, FaChevronRight, FaClock } from 'react-icons/fa6';
        ...
        <div className="space-y-4">
            ...
            <form onSubmit={handleStripeSubmit} className="space-y-4">
                ...
                <Button type="submit" className="w-full" disabled={isProcessingPayment || !stripe || !elements}>
                    {isProcessingPayment
                        ? 'Paiement en cours...'
                        : `Payer ${bookingAmount.toFixed(2)} ${bookingCurrency === 'USD' ? '$' : '€'}`}
                </Button>
            ...
            {isProcessingPayment ? (
                ...
                <div className="space-y-3 p-4">
                    ...
                    <p className="text-xs text-muted-foreground">
                        Verifiez la console pour plus de details ou contactez l'administrateur.
                    </p>
...
function BookingForm() {
    ...
    const validateServiceArea = (placeToValidate = selectedPlace) => {
        ...
        if (distance > maxRadius) {
            toast.error(
                `Désolé, cette adresse se trouve à ${distance.toFixed(1)} km de notre zone de service. Nous intervenons uniquement dans un rayon de ${maxRadius} km autour de ${companyData?.serviceArea?.primary}.`
            );
            return false;
        }
    ...
    const isStripeAvailable =
        storeSettings?.paymentMethods?.stripe?.enabled && storeSettings?.paymentMethods?.stripe?.apiPuplicKey;
    const isSumUpAvailable =
        storeSettings?.paymentMethods?.sumup?.enabled && storeSettings?.paymentMethods?.sumup?.merchantCode;
    ...
    useEffect(() => {
        ...
        void (async () => {
            ...
            try {
                await withTimeout(sendBookingEmails(createdBooking), 15000, 'Email timeout after booking confirmation');
            } catch (emailError) {
        ...
        if (paymentOption === 'pay_now') {
            if (paymentGateway === 'stripe') {
                if (!isStripeReady || !stripeOptions) {
                    toast.error(
                        'Le paiement Stripe est indisponible. Veuillez choisir une autre methode ou Paiement plus tard.'
                    );
                    return;
                }
                ...
                {stripeOptions && stripePromise && isStripeReady ? (
                    ...
                    <div className="space-y-3 p-4">
                        <p className="text-sm text-muted-foreground">
                            Le paiement en ligne est indisponible actuellement.
                        </p>
                        <Button
                            type="button"
                            variant="outline"
            ...
            <div className="booking-form-container p-2">
                ...
                <div className="rounded-lg border border-border bg-muted/20 p-4 space-y-3">
                    ...
                    <Button
                        type="button"
                        className="w-full"
                        onClick={() => router.push(`/booking/success?id=${createdBooking.id}`)}>
                        ...
                        <label
                            ...
                            <div className="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        Passez au paiement en ligne apres validation du formulaire.
                                    </p>
                                </div>
                                <span
                                    className={`h-4 w-4 rounded-full border-2 ${paymentOption === 'pay_now' ? 'border-primary bg-primary' : 'border-muted-foreground/40'}`}
                                />
                            </div>
                        </label>
                                ...
                                {isStripeAvailable && bothPaymentMethodsAvailable && (
                                    <label
                                        ...
                                        <div className="flex items-center justify-between gap-2">
                                            ...
                                            <span
                                                className={`h-3 w-3 rounded-full border-2 ${paymentGateway === 'stripe' ? 'border-primary bg-primary' : 'border-muted-foreground/40'}`}
                                            />
                                        </div>
                                    </label>
                                )}
                                ...
                                {isSumUpAvailable && bothPaymentMethodsAvailable && (
                                    <label
                                        ...
                                        <div className="flex items-center justify-between gap-2">
                                            ...
                                            <span
                                                className={`h-3 w-3 rounded-full border-2 ${paymentGateway === 'sumup' ? 'border-primary bg-primary' : 'border-muted-foreground/40'}`}
                                            />
                                        </div>
                                    </label>
                                )}
                        ...
                        <label
                            ...
                            <div className="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        Confirmez la reservation maintenant et reglez ulterieurement.
                                    </p>
                                </div>
                                <span
                                    className={`h-4 w-4 rounded-full border-2 ${paymentOption === 'pay_later' ? 'border-primary bg-primary' : 'border-muted-foreground/40'}`}
                                />
                            </div>
                        </label>
                    </div>
